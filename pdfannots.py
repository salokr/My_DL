...
dictionary = {}
full_labeled_text = ''
dict_rgb_to_name = {'[0.568627451, 0.9529411765, 1]' : 'Objective', '[1, 0.9294117647, 0.8078431373]':'Solution/Novel Approach', '[0.9843137255, 0.5254901961, 1]':'Motivation', '[1, 0.5254901961, 0.2117647059]':'Current Situation and it\'s Problem', '[0.1960784314, 1, 0.0352941176]':'New Observation', '[1, 1, 0]':'Context', '[0.262745098, 0.568627451, 1]':'Technical Advancement', '[1, 0.1529411765, 0.0588235294]':'Organization'}
...
class Annotation:
    def __init__(self, pageno, tagname, color,coords=None, rect=None, contents=None):
        self.color = color
        self.pageno = pageno
...
def getannots(pdfannots, pageno):
    ...
    for pa in pdfannots:
        ...
        a = Annotation(pageno, subtype.name.lower(), pa['C'],pa.get('QuadPoints'), pa.get('Rect'), contents)
        #print('=========================================================')
        #print(a.gettext())
        #print('=========================================================')
        annots.append(a)
    #print(dir(annots[0]))
    #print(type(annots[0]).__name__)
    #print("->" + str(annots[0].boxes[0].__index__()) + "<-")
    #print(type(annots[0].capture()))
    #print(type(annots[0].capture()))
    #print(dir(annots[0].boxes[0]))
    #print((annots[0].gettext()))
    #print((annots[0].text))
    return annots
...
def process(s):
    ...
    hexa_list=regex_hexadecimal.findall(s)
...
def prettyprint(annots, outlines, mediaboxes):
    global full_labeled_text
    fix_encoding = lambda s: (process(s)) 

    tw = textwrap.TextWrapper(width=80, initial_indent=" * ", subsequent_indent="   ")

    def fmtpos(annot):
        apos = annot.getstartpos()
        #print(str(dir(annot)) + str(dir(annot.tagname)) + ' -> ' + str(annot.tagname.()) + '<-')
        if apos:
    ...
    def printitem(*args):
        ...
        msg = ' '.join([fix_encoding(s) for s in args])
        #print(tw.fill(msg) + "\n")
    ...
    if comments:
        ...
        for a in comments:
            text = fmttext(a)
            #print(dir(a.tagname.__getnewargs__()))
            #print(type(a))
            col = str(a.color)
            if text:
                ...
                if dictionary.get(col) is None:
                    dictionary[col] = []
                tt = process(text).encode('ascii', 'ignore')
                dictionary[col].append(tt)
                full_labeled_text = full_labeled_text + '<<|Content|>>' + tt + '<<|Label|>>' +  dict_rgb_to_name[col] + '<<|END|>>' 
...
def process_finally(filename, full_labeled_text):
    #print(filename)
    annotations = full_labeled_text.split('<<|END|>>')
...
def main():
    ...
    #for keys, items in dictionary.items():
        ...
        #print(dict_rgb_to_name[keys])
        #print(dictionary[keys])
        #print('__________________________________________________________________________')
    print(len(dict_rgb_to_name))
    process_finally(sys.argv[1], full_labeled_text)
    #print(full_labeled_text)
